    ...
    setBankingData(prevData => {
      ...
      if (transaction.type === 'transfer') {
        if (transaction.fromAccount.startsWith('savings-')) {
          const fromSavingsAccountName = transaction.fromAccount.replace('savings-', '');
          newData.savings[fromSavingsAccountName].balance -= transaction.amount;
        } else {
          newData[transaction.fromAccount as keyof BankingData].balance -= transaction.amount;
        }

        if (transaction.toAccount.startsWith('savings-')) {
          const toSavingsAccountName = transaction.toAccount.replace('savings-', '');
          newData.savings[toSavingsAccountName].balance += transaction.amount;
        } else {
          newData[transaction.toAccount as keyof BankingData].balance += transaction.amount;
        }
      } else {
        ...
        if (accountType.startsWith('savings-')) {
          ...
          newData[accountType as keyof BankingData].balance += transaction.type === 'deposit' ? transaction.amount : -transaction.amount;
        }
      }
      ...
      if (transaction.fromAccount.startsWith('savings-')) {
        ...
        (newData[transaction.fromAccount as keyof BankingData] as AccountData).transactions.unshift(newTransaction);
      }
      ...
      if (transaction.toAccount && transaction.toAccount !== transaction.fromAccount) {
        if (transaction.toAccount.startsWith('savings-')) {
          ...
          (newData[transaction.toAccount as keyof BankingData] as AccountData).transactions.unshift(newTransaction);
        }
      }
