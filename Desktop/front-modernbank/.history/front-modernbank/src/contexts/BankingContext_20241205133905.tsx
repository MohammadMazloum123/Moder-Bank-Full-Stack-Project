    ...
    const addTransaction = (transaction: Omit<Transaction, 'id' | 'date'>): { success: boolean; message: string } => {
      ...
      // };
        ...
        const addSavingsAccount = async (name: string, goalName: string, goalAmount: number): Promise<{ success: boolean; message: string }> => {
            try{
              const response = await axiosInstance.post('/banking/savings', {name, goalName, });
        ...
        const contributeSavings = (accountName: string, amount: number): Promise<{ success: boolean; message: string } => {
