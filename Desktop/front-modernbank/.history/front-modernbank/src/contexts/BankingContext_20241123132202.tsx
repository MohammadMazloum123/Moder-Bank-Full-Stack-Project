...
export const BankingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const mockData: BankingData = {
        ...
        expenses: [
            ...
            { id: 2, description: 'Electric Bill', amount: 80, category: 'Utilities', date: new Date().toISOString() },
            ],
        };
...
useEffect(() => {
    ...
    const addTransaction = (transaction: Omit<Transaction, 'id' | 'date'>): { success: boolean; message: string } => {
        ...
        return setBankingData(prevData => {
          ...
          try {
            if (transaction.type === 'transfer' || transaction.type === 'withdrawal') {
              ...
              if (fromBalance < transaction.amount) {
                return {
                  ...prevData,
                  error: { success: false, message: 'Insufficient balance for the transaction.' }
                };
              }
            }
            ...
            return {
              ...newData,
              error: { success: true, message: 'Transaction completed successfully.' }
            };
          } catch (error) {
            console.error('Error processing transaction:', error);
            return {
              ...prevData,
              error: { success: false, message: `Error processing transaction: ${(error as Error).message}` }
            };
          }
        });
      };
